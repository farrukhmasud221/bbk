...
for (let i of files) {
    ...
    for (let pos of viewsObjPos) {
        ...
        while (error) {
            try {
                ...
                let separatorIndices = [viewsString.indexOf(',', viewsString.indexOf(undefinedVar)), viewsString.indexOf(' ', viewsString.indexOf(undefinedVar)), viewsString.indexOf('+', viewsString.indexOf(undefinedVar))];
                // let commaIndex = viewsString.indexOf(',', viewsString.indexOf(undefinedVar));
                ...
                undefinedVar = viewsString.substring(viewsString.indexOf(undefinedVar), separatorIndices.sort().find(element => element >= 0));
                //console.log(undefinedVar)
                undefinedObj[undefinedVar] = Math.random();
    ...
    fs.writeFileSync('./target/' + i, viewsObjPos.length ? finalString : code);
}